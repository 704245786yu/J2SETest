...
public class PinYin4j {
	...
	/**获取拼音字符串，对于多音字则默认使用第一种读音
	 * @author zhiyu
	 * @param chinese 中文
	 */
	public String getPinyinStr(String chinese) {
		...
		for (int i = 0; i < srcChar.length; i++) {
			...
			// 是中文或者a-z或者A-Z转换拼音
			if (String.valueOf(c).matches("[\\u4E00-\\u9FA5]+")) {
				String[] t = PinyinHelper.getUnformattedHanyuPinyinStringArray(c);//返回的数据里包含了一个字的多种读音
				pinyin += t[0].substring(0, t[0].length()-1);
			}
		}
		return pinyin;
	}
